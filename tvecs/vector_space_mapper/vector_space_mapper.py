...
class VectorSpaceMapper(object):
    ...
    def get_cosine_similarty_same_lang(self, word1, word2, lang):
        print lang
        if lang == 'en':
            try:
                vec_1 = self.model_1[word1]
                vec_2 = self.model_1[word2]
                return 1 - dist.cosine(vec_1, vec_2)
            except KeyError :
                return None
        else :
            try:
                vec_1 = self.model_2[word1]
                vec_2 = self.model_2[word2]
                return 1 - dist.cosine(vec_1, vec_2)
            except KeyError :
                return None

    def get_most_similar_word_same_lang(self, word):
        synonym = None
        try:
        ...
        if self.lt is not None:
            try:
                synonym = self.model_1.most_similar(
                    positive=[
    ...
    def get_recommendations_from_word(self, word, topn=10, pretty_print=False):
        ...
        if self.lt is not None:
            try:
                ...
                lemmas = set(chain.from_iterable([word.lemma_names() for word in synonym]))
                if len(lemmas) > 0:
                    print "Synonym found", lemmas
                for syn in lemmas: 
                    print syn
                    try:
                        data = self.model_2.most_similar(
                            positive=[
                                self._predict_vec_from_word(syn)
                            ],
                            topn=topn
                        ) 
